function renderGameList(parent) {
  ...
  renderHeader(parent);
  renderGameListNav(parent);

  client.socket.send(JSON.stringify({ event: "cl:getAllGames", data: null }));
}
...
function renderActiveGameCard(parent, gameObj) {
  const container = document.createElement("div");
  ...
  container.innerHTML = `
    <div id="nameAndTime">
        <p id="gameNameActive">${gameObj.name}</p>
        <p id="time"> 12:30 left (IMPLEMENT THIS)</p>
    </div>
    <div id="roundsAndPeople">
        <div id="rounds">
            ...
            <p id="roundsP">${gameObj.roundCount}</p>
        </div>
        <div id="people">
            ...
            <p id="peopleP">${gameObj.playerCount}</p>
        </div>
    </div>
    <div id="gameCardTags">
        <div id="guessTag">
